...
const AdminPanelItem = ({
  ...
  setProducts,
  index,
}: AdminPanelItemProps): ReactElement => {
  ...
  const handleCategoryChange = (
    ...
    const updatedProducts: IProduct[] = [...products];
    if (
      updatedProducts[index][field].includes(value) &&
      updatedProducts[index][field].length === 1
    )
      return;
    else if (updatedProducts[index][field].includes(value)) {
      (updatedProducts[index][field] as string[]).splice(
        updatedProducts[index][field].indexOf(value),
        1
      );
    } else {
      (updatedProducts[index][field] as string[]).push(value);
    }
  ...
  const handleDelete = (item: IProduct): void => {
    ...
    setLocalStorageItem(
      "products",
      products.filter((product) => product !== item)
    );
  };
  ...
  return (
    <div className="admin-panel__item">
      <div className="admin-panel__item-preview">
        <div className="admin-panel__item-img">
          ...
          <div className="filter-types__container">
            {filterTypes.map((filterType) => (
              <div key={filterType} className="filter-types__item">
                <input
                  className="filter-types__item-checkbox"
                  type="checkbox"
                  name={filterType}
                  value={filterType}
                  onChange={(e) => {
